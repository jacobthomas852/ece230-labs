...
module test();
    ...
    task test_half_sub(input a, input b);
        ...
        begin
            ...
            if (leds[0] === expected_y && leds[1] === expected_b_out) 
                ...
                $error("half_sub Test Failed: A=%b, B=%b => Y=%b, B_out=%b (Expected Y=%b, B_out=%b)", 
    ...
    task test_ones_compliment(input [3:0] a, input [3:0] b, input [3:0] expected_y);
        begin
            ...
            if (leds[5:2] === expected_y) 
                ...
                error_count = error_count + 1;
                $error("ones_compliment Test Failed: A=%b, B=%b => Y=%b (Expected Y=%b)", 
    ...
    task test_twos_compliment(input [7:0] a);
        ...
        begin
            ...
            if (leds[13:6] === expected_y) 
                ...
                error_count = error_count + 1;
                $error("twos_compliment Test Failed: A=%b => Y=%b (Expected Y=%b)", 
    ...
    initial begin
        ...
        test_ones_compliment(4'b0101, 4'b0010, 4'b0111);    //(+5) + (+2) = (+7)
        test_ones_compliment(4'b1010, 4'b0010, 4'b1100);    //(-5) + (+2) = (-3)
        test_ones_compliment(4'b0101, 4'b1101, 4'b0011);    //(+5) + (-2) = (+3)
        test_ones_compliment(4'b1010, 4'b1101, 4'b1000);    //(-5) + (-2) = (-7)
        ...
        //Check if there are any failing testcases
        if (error_count > 0)
            $error("Failing Testcases! Check above messages for failing cases and fix them.");
        else
